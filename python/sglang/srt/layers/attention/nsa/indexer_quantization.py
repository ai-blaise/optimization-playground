"""NSA indexer quantization layout helpers."""
...
def get_nsa_indexer_quant_method(server_args: Any) -> str:
...
def get_nsa_indexer_cache_layout(
    quant_method: Optional[str], index_head_dim: int
) -> NSAIndexerCacheLayout:
    ...
    if index_head_dim != 128:
        raise ValueError(
            f"NSA indexer cache layout only supports head_dim=128, got {index_head_dim}."
        )

    if method == INDEXER_FP8_QUANT_METHOD:
        return NSAIndexerCacheLayout(
            quant_method=method,
    ...
    if method == INDEXER_DISABLED_QUANT_METHOD:
        return get_nsa_indexer_cache_layout(INDEXER_FP8_QUANT_METHOD, index_head_dim)

    raise ValueError(f"Unsupported NSA indexer quantization method: {method!r}.")
