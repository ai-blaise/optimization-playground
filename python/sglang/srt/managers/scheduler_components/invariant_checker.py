...
class SchedulerInvariantChecker:
    ...
    get_smc_manager: Callable
    get_smc_resampler: Callable
    count_req_pool_leak_warnings: int = 0
    count_memory_leak_warnings: int = 0

    def _smc_held_tokens(self) -> int:
        smc_manager = self.get_smc_manager()
    ...
    def _check_pool_invariant(
        ...
        """Check memory accounting for a token pool."""
        total_accounted = (
    ...
    def _report_leak(self, pool_name: str, token_msg: str):
        ...
        smc_manager = self.get_smc_manager()
        smc_resampler = self.get_smc_resampler()
        ...
        if smc_resampler is not None:
            msg += (
                ...
                f"pending_new_groups={list(smc_resampler._pending_new_groups)}\n"
            )
        raise_error_or_warn(
            self,
            envs.SGLANG_ENABLE_STRICT_MEM_CHECK_DURING_IDLE.get(),
