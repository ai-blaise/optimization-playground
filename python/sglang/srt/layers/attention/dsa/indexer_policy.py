...
DSA_INDEXER_MODES = ("vanilla", "indexcache", "hisa", "indexcache-hisa")
...
def validate_indexcache_pattern(pattern: Optional[str], num_layers: int) -> None:
    ...
    if len(pattern) != num_layers:
        raise ValueError(
            f"DSA IndexCache pattern length must match num_hidden_layers: "
            f"got {len(pattern)}, expected {num_layers}."
        )
    if any(role not in ("F", "S") for role in pattern):
        raise ValueError("DSA IndexCache pattern may only contain 'F' and 'S'.")
    if pattern[0] != "F":
        raise ValueError("DSA IndexCache pattern must keep layer 0 as 'F'.")
...
def get_indexcache_skip_flags(
    *,
    ...
    if freq < 1:
        raise ValueError("DSA IndexCache frequency must be at least 1.")
