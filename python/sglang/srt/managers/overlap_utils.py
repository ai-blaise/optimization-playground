...
class FutureMap:
    ...
    def _lazy_init_smc_buf(self, draft_input) -> None:
        self._forward_buf_initialized = True
    ...
    def set_input_ids_sentinel(
        ...
        # fall back to materializing batch.input_ids from the SMC spec_info
        # so the scheduler's downstream resolve_future / kernel prep keeps
        # working without a sentinel.
        if future_indices is None:
            next_draft_input = batch.spec_info
            if next_draft_input is not None:
                token_ids = (
                    next_draft_input.verified_id
                    if hasattr(next_draft_input, "verified_id")
                    else getattr(next_draft_input, "last_token_ids", None)
                )
    ...
    def stash(
        ...
        if self.spec_algo.is_smc():
            if not self._forward_buf_initialized:
                self._lazy_init_smc_buf(payload)
            self.last_token_ids_buf[indices] = payload.verified_id.to(
                self.last_token_ids_buf.dtype
            )
            self.new_seq_lens_buf[indices] = payload.new_seq_lens.to(
