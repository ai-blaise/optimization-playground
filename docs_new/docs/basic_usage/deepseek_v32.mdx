...
The `ai-blaise/optimization-playground` fork includes additional opt-in paths for the REAP DeepSeek V3.2 lane. The active target is the Hugging Face config-declared stack in `BlaiseAI/DeepSeek-V3.2-REAP-345B-SpinQuant-ActKV-NVFP4`:

- **G1 attention gate and GatedNorm**: `attention_output_gate=true`, `gated_norm=true`, and `gated_norm_rank=16` in `config.json` construct the G1 gate and GatedNorm modules. On Blackwell GPUs, BF16 GatedNorm dispatches to `sgl_kernel.gated_norm_cute_forward` when the local `sgl-kernel` wheel includes the registered op; documented shape/debug fallbacks remain available through `SGLANG_GATED_NORM_USE_TRITON=1` and `SGLANG_GATED_NORM_DISABLE_CUTE=1`.
- **HIGGS dense MLA KV**: `quantization_config.kv_cache_scheme.quant_method=higgs_dense_2bit` enables the 258-byte HIGGS dense MLA KV pool and fused decode path. This replaces the older TurboQuant active lane for REAP unless a benchmark explicitly reopens TurboQuant.
- **NVFP4 IndexCache+HISA**: `quantization_config.indexer_quantization.quant_method=nvfp4_e2m1_ue8m0` plus `hisa.enabled=true` and `hisa.mode=indexcache-hisa` selects the Blackwell NVFP4 indexer layout and the combined IndexCache+HISA model path. A nested `indexcache` block can still provide a custom frequency or pattern; otherwise the launch default is used.
- **SMC-SD**: `--speculative-algorithm SMC` remains a serving policy, not a checkpoint property. Run it on decode workers only with the FP8 draft model and `--smc-draft-kv-cache-dtype fp8_e4m3`; do not infer draft-model orchestration from the target model's HF config.

The Dynamo production profile is a 4-GPU prefill plus 4-GPU decode deployment. It enables DSA sparse attention, HF-declared HIGGS dense MLA KV, HF-declared NVFP4 IndexCache+HISA, Dynamo KV-aware routing, and decode-only SMC-SD. Do not enable SGLang HiCache in that profile unless the no-radix/cache-transfer contract is deliberately reworked and revalidated.
...
Example REAP launch using the target model's HF config for HIGGS and NVFP4 IndexCache+HISA:
...
Example REAP launch with HF-declared HIGGS/NVFP4 IndexCache+HISA plus SMC-SD:
...
HIGGS currently requires target BF16 source KV dtype; `kv-cache-dtype=auto` is promoted to BF16 when the HF config declares `higgs_dense_2bit`. The REAP combo runner defaults the SMC draft model to `--smc-draft-kv-cache-dtype fp8_e4m3`; set the draft-only override instead of changing the global target KV dtype.

The SMC-SD path was brought in from the reference SMC-SD SGLang integration and adapted for the current `ForwardBatch`/worker APIs, FP8 draft loading, DeepSeek V3.2 REAP TP=8/DP=8 with DP attention, and the HF-declared HIGGS plus NVFP4 IndexCache+HISA target stack. The current 16K/4K gate improves end-to-end throughput over vanilla MTP by about 7.0%, but still regresses mean TTFT by about 6.7%, so treat SMC-SD as the primary experimental optimization path until the TTFT gate is closed.

NVFP4 IndexCache+HISA is the active indexer lane for the Blaise checkpoint. HISA remains paper-aligned with block top-k 64 and index top-k 2048; the optimization-playground variant broadens long-prefill dispatch coverage above the HISA boundary while preserving ordinary IndexCache behavior below that boundary and when the config disables HISA. Rerun the Blackwell benchmark gates before changing production throughput claims for the HIGGS/NVFP4-HISA stack.
