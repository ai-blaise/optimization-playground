...
def validate_layersplit_server_args(
    *,
    ...
    is_deepseek_dsa_model: bool,
    enable_turboquant_dense_kv_cache: bool,
    turboquant_skip_layers,
) -> None:
    if not enable_dsa_prefill_context_parallel:
        raise ValueError(
            "--dsa-prefill-cp-kv-storage-mode=layersplit requires "
            "--enable-dsa-prefill-context-parallel."
        )
    if attn_cp_size <= 1:
        raise ValueError(
            "--dsa-prefill-cp-kv-storage-mode=layersplit requires an "
    ...
    if disaggregation_mode == "prefill" and not all_cp_ranks_transfer:
        raise ValueError(
            "--dsa-prefill-cp-kv-storage-mode=layersplit with "
    ...
    if not is_deepseek_dsa_model:
        raise ValueError(
            "--dsa-prefill-cp-kv-storage-mode=layersplit is only "
...
class LayerSplitPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if self.layout not in DSA_PREFILL_CP_LAYERSPLIT_LAYOUT_CHOICES:
