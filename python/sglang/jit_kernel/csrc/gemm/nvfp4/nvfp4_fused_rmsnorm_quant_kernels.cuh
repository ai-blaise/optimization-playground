...
//  layernorm(hidden_states) at L1013 of communicator.py).
// ============================================================================
//
// In that branch the residual has already been folded into `hidden_states`
// before the cross-DP allgather (attn_tp_rank==0 path at L997), so the
// layernorm here is a pure RMSNorm — NO residual-add.
//
// Outputs:
//   * y_out  [m, n]   BF16 post-norm hidden_states (NEW tensor, no in-place)
//   * fp4_out [m, n/2] uint8 packed E2M1 — stash for the next MoE
//                      apply_weights call (skips its fp4_quantize)
//   * sf_out  [m, n/16] fp8_e4m3 (int32-packed) — companion scales
//
// Producing both BF16 and FP4 is the deploy-win condition: the gate
// router (BF16 GEMM) reads `y_out`, and the MoE expert apply_weights
// reads `(fp4_out, sf_out)` via the iter3 stash hook on the tensor
// (compressed_tensors_w4a4_nvfp4_moe.py).
//
// One CTA per row. Single-phase: load x, accumulate sum-of-squares,
// cooperative-group reduce, normalize+weight, write both BF16 + FP4.
template <class Type, bool kCastXBeforeOutMul = false, bool UE8M0_SF = false>
__global__ void
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
__launch_bounds__(512, 4) cvt_fused_rmsnorm_only_to_fp4_linear(
#else
cvt_fused_rmsnorm_only_to_fp4_linear(
#endif
    int32_t numRows, int32_t numCols, Type const* __restrict__ input,
    Type const* __restrict__ weight, float const* SFScale,
    Type* __restrict__ y_out, uint32_t* fp4_out, uint32_t* SFout,
    float eps) {
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 1000)
  using packed_t = typename ::dtype_trait<Type>::packed_t;
  using PackedVec = PackedVec<Type>;
  ...
  // Per-thread register cache: PackedVec input (BF16) and weight (BF16).
  // No residual, no cross-iteration cache needed for the second phase
  // because we only need x (or its f32 expansion) once; we keep it in
  // registers for the rsqrt + normalize step.
  packed_t cached_inp[kFusedQuantMaxVecsPerThread][4];
  PackedVec cached_weight[kFusedQuantMaxVecsPerThread];
  float2 cached_f2[kFusedQuantMaxVecsPerThread][4];  // for kCastXBeforeOutMul
  ...
  for (int rowIdx = blockIdx.x; rowIdx < numRows; rowIdx += gridDim.x) {
    ...
    // --- Phase 1: load x, accumulate squared sum, stash for phase 2 ---
    float thread_sq_sum = 0.0f;
    ...
    for (int colIdx = threadIdx.x, iter = 0;
      ...
      for (int i = 0; i < 4; ++i) {
        float2 xf = device::cast<fp32x2_t>(x_vec.elts[i]);
        thread_sq_sum += xf.x * xf.x + xf.y * xf.y;
        cached_inp[iter][i] = x_vec.elts[i];
        if constexpr (kCastXBeforeOutMul) {
          cached_f2[iter][i] = xf;
        }
      }
    ...
    // --- Phase 2: normalize, weight-multiply, write BF16 y_out + FP4. ---
    int iter = 0;
    for (int colIdx = threadIdx.x; colIdx < numColVecs;
         colIdx += blockDim.x, ++iter) {
      PackedVec y_vec;
      ...
      for (int i = 0; i < 4; ++i) {
        ...
        if constexpr (kCastXBeforeOutMul) {
          ...
          vf = device::cast<fp32x2_t>(cached_inp[iter][i]);
          float2 wf = device::cast<fp32x2_t>(cached_weight[iter].elts[i]);
          y_vec.elts[i] = device::cast<packed_t>(make_float2(
              vf.x * wf.x * inv_rms, vf.y * wf.y * inv_rms));
        }
      ...
      // Write BF16 hidden_states for the gate/router/shared_experts
      // consumer (those don't read FP4).
      p_y_out[colIdx] = y_vec;

      // Write FP4 + SF for the MoE expert apply_weights consumer (stash
      // is picked up via the iter3 `_sglang_pre_quantized_fp4` hook on
      // the y_out tensor).
      int64_t outOffset =
          static_cast<int64_t>(rowIdx) * numColVecs + colIdx;
      auto sf_out =
...
// Launcher for the residual-less kernel. Mirrors invokeFusedRMSNormFP4QuantLinear
// but without the residual pointer; reuses the same wide/narrow dispatch
// logic and per-thread register cap to stay inside the same envelope.
template <typename T>
void invokeFusedRMSNormOnlyFP4QuantLinear(int m, int n, T const* input,
                                          T const* weight,
                                          float const* SFScale, T* y_out,
                                          int64_t* fp4_output,
                                          ...
                                          DLDevice device) {
  // Narrow path only for iter4 P0 — covers production H=7168, m=1..512.
  // Wide variant lift to be added in iter4.5 if m>=256 regression
  // appears (the residual-less variant is lighter on register pressure
  // than the residual one so the wide path may not be needed at all).
  ...
  int vecs_per_thread =
      (numColVecs + block_threads - 1) / block_threads;
  RuntimeCheck(vecs_per_thread <= kFusedQuantMaxVecsPerThread,
               "fused RMSNorm-only FP4: vecs_per_thread ", vecs_per_thread,
  ...
  if (useUE8M0) {
    if (cast_x_before_out_mul) {
      auto k = cvt_fused_rmsnorm_only_to_fp4_linear<T, true, true>;
      launcher(k, m, n, input, weight, SFScale, y_out, out_u32, sf_u32, eps);
    } else {
      auto k = cvt_fused_rmsnorm_only_to_fp4_linear<T, false, true>;
      launcher(k, m, n, input, weight, SFScale, y_out, out_u32, sf_u32, eps);
    }
  } else {
    if (cast_x_before_out_mul) {
      auto k = cvt_fused_rmsnorm_only_to_fp4_linear<T, true, false>;
      launcher(k, m, n, input, weight, SFScale, y_out, out_u32, sf_u32, eps);
    } else {
      auto k = cvt_fused_rmsnorm_only_to_fp4_linear<T, false, false>;
      launcher(k, m, n, input, weight, SFScale, y_out, out_u32, sf_u32, eps);
    }
  }
}

template void invokeFusedRMSNormOnlyFP4QuantLinear(int m, int n,
                                                   half const* input,
                                                   half const* weight,
                                                   float const* SFScale,
                                                   half* y_out,
...
template void invokeFusedRMSNormOnlyFP4QuantLinear(
    int m, int n, __nv_bfloat16 const* input, __nv_bfloat16 const* weight,
    float const* SFScale, __nv_bfloat16* y_out, int64_t* fp4_output,
...
// Iter4 PRIMARY: residual-less fused RMSNorm + linear FP4 quantize.
// Public entry for the
// CommunicateWithAllReduceAndLayerNormFn._gather_hidden_states_and_residual
// `not use_layer_norm_before_gather` branch — emits BOTH BF16
// hidden_states (for gate/router/shared_experts) and FP4+SF (stash for
// MoE expert apply_weights). See top-of-file comment for the math.
void fused_rmsnorm_only_to_fp4_linear_sm100a_sm120a(
    tvm::ffi::TensorView input, tvm::ffi::TensorView weight,
    tvm::ffi::TensorView input_sf, tvm::ffi::TensorView y_output,
    tvm::ffi::TensorView fp4_output, tvm::ffi::TensorView sf_output,
    double eps, bool cast_x_before_out_mul, bool enable_pdl) {
  RuntimeCheck(input.device().device_type == kDLCUDA,
               "input must be a CUDA tensor");
  ...
  const int device_id = input.device().device_id;
  RuntimeCheck(getSMVersionFusedQuant(device_id) >= 100,
               "fused_rmsnorm_only_to_fp4_linear requires sm100+");
  ...
  if (host::is_type<fp16_t>(input.dtype())) {
    ...
    invokeFusedRMSNormOnlyFP4QuantLinear(
