...
def _resolve_speculative_algorithm_alias(
    ...
    if (
        speculative_algorithm in ("NEXTN", "EAGLE", "EAGLE3")
        and speculative_draft_model_path
    ):
...
def handle_speculative_decoding(server_args: "ServerArgs") -> None:
    ...
    elif server_args.speculative_algorithm == "SMC":
...
def _handle_smc(server_args: "ServerArgs") -> None:
    ...
    if server_args.model_path.lower() in ["none", "dummy"]:
        pass
    elif is_deepseek_nsa(server_args.get_model_config().hf_config):
    ...
    if (
        ...
        unsupported_attention_backends["speculative_draft_attention_backend"] = (
            draft_attention_backend
        )
