...
- **TokenSpeed MLA boundary**: `--attention-backend tokenspeed_mla` is a dense MLA backend for compatible Blackwell MLA models. It is not a DSA selected-page kernel and must not be combined with this DeepSeek V3.2 REAP NSA stack, HiSparse, IndexCache, dense TurboQuant, or LayerSplit. Use `scripts/playground/run-tokenspeed-mla-ab.sh` to compare TokenSpeed against dense MLA backends on compatible models, and keep `--attention-backend nsa` for this REAP production profile.
