...
class TestWarpDecodeIntegration(unittest.TestCase):
    ...
    def test_env_var_disabled_by_default(self):
        ...
        old_val = os.environ.pop("SGLANG_ENABLE_WARP_DECODE", None)
        try:
            # Force reload
            from sglang.srt.layers.moe.warp_decode import integration
            integration._WARP_DECODE_ENABLED = None
            integration._ensure_config()
            self.assertFalse(integration._WARP_DECODE_ENABLED)
        finally:
            if old_val is not None:
                os.environ["SGLANG_ENABLE_WARP_DECODE"] = old_val

    def test_env_var_enabled(self):
        """Verify warp decode can be enabled via env var."""
        import os
        old_val = os.environ.get("SGLANG_ENABLE_WARP_DECODE")
        try:
            os.environ["SGLANG_ENABLE_WARP_DECODE"] = "1"
            from sglang.srt.layers.moe.warp_decode import integration
            integration._WARP_DECODE_ENABLED = None
            integration._ensure_config()
            self.assertTrue(integration._WARP_DECODE_ENABLED)
        finally:
            if old_val is not None:
                os.environ["SGLANG_ENABLE_WARP_DECODE"] = old_val
            else:
                os.environ.pop("SGLANG_ENABLE_WARP_DECODE", None)

    def test_max_batch_threshold(self):
        """Verify batch size threshold is respected."""
        import os
        old_max = os.environ.get("SGLANG_WARP_DECODE_MAX_BATCH")
        old_enable = os.environ.get("SGLANG_ENABLE_WARP_DECODE")
        try:
            os.environ["SGLANG_ENABLE_WARP_DECODE"] = "1"
            os.environ["SGLANG_WARP_DECODE_MAX_BATCH"] = "16"
            from sglang.srt.layers.moe.warp_decode import integration
            integration._WARP_DECODE_ENABLED = None
            integration._WARP_DECODE_MAX_BATCH = None
            integration._ensure_config()

            self.assertEqual(integration._WARP_DECODE_MAX_BATCH, 16)
        finally:
            if old_max is not None:
                os.environ["SGLANG_WARP_DECODE_MAX_BATCH"] = old_max
            else:
                os.environ.pop("SGLANG_WARP_DECODE_MAX_BATCH", None)
            if old_enable is not None:
                os.environ["SGLANG_ENABLE_WARP_DECODE"] = old_enable
            else:
                os.environ.pop("SGLANG_ENABLE_WARP_DECODE", None)
