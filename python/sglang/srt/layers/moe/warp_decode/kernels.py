...
# ==============================================================================
"""Warp Decode Triton kernels for MoE small-batch decode.

Implements the two-kernel warp decode approach:

Kernel 1 (gate_up): Each program computes ONE intermediate neuron for
  one (token, expert) pair. The activation vector is streamed once,
  producing both gate and up dot products in FP32, then SiLU(gate)*up
  is written as one intermediate value.

Kernel 2 (down): Each program computes ONE output dimension for one
  token. It loops over top-k routed experts, folding routing weights
  into a single FP32 accumulator. No cross-program synchronization.

Supports:
  - BF16 weights and activations
  - FP8 (E4M3) weights with per-tensor or blockwise scales
  - NVFP4 weights with blockscales (uint8 packed, 2 values per byte)

Targets SM100 (B200, 232KB SMEM, 148 SMs) but works on SM80+.
"""
...
# ---------------------------------------------------------------------------
# Kernel 2b: Down projection for NVFP4-packed w2 weights
# ---------------------------------------------------------------------------
# w2 is stored as [num_experts, hidden_size, intermediate_size // 2] in uint8
# with blockscales. This variant dequantizes on-the-fly.
# ---------------------------------------------------------------------------

@triton.jit
def _warp_decode_down_fp4_kernel(
    # Intermediate activations [num_tokens * top_k, intermediate_size]
    intermediate_ptr,
    intermediate_stride_te: int,
    intermediate_stride_n: int,
    # Down projection weight (NVFP4 packed) [num_experts, hidden_size, intermediate_size // 2]
    w_down_ptr,
    w_down_stride_e: int,
    w_down_stride_d: int,
    w_down_stride_n: int,
    # Block scales [num_experts, hidden_size, intermediate_size // group_size]
    w_down_scale_ptr,
    w_down_scale_stride_e: int,
    w_down_scale_stride_d: int,
    w_down_scale_stride_g: int,
    # Global scale (per-tensor) for down projection
    w_down_alpha_ptr,
    # Routing weights [num_tokens, top_k]
    routing_weights_ptr,
    routing_stride_t: int,
    ...
    """Down projection with NVFP4 weight dequantization."""
    ...
    # Load global alpha scale
    w_alpha = tl.load(w_down_alpha_ptr).to(tl.float32)
...
# ---------------------------------------------------------------------------
# Autotuning configs for SM100 (B200)
# ---------------------------------------------------------------------------
# B200: 148 SMs, 232 KB shared memory per SM
# For decode batch sizes 1-64, we want high occupancy with small blocks.
# ---------------------------------------------------------------------------

def _get_gate_up_autotune_configs():
    """Autotuning configurations for gate/up kernel."""
    return [
        triton.Config(
...
def _get_down_autotune_configs():
    """Autotuning configurations for down kernel."""
    return [
        triton.Config(
            {"BLOCK_D": 8, "BLOCK_N": 128},
            num_warps=4,
            num_stages=2,
        ),
        triton.Config(
            {"BLOCK_D": 16, "BLOCK_N": 128},
            num_warps=4,
            num_stages=2,
        ),
        triton.Config(
            {"BLOCK_D": 32, "BLOCK_N": 64},
            num_warps=4,
            num_stages=3,
        ),
        triton.Config(
            {"BLOCK_D": 8, "BLOCK_N": 256},
            num_warps=8,
            num_stages=2,
        ),
        triton.Config(
            {"BLOCK_D": 16, "BLOCK_N": 256},
            num_warps=8,
            num_stages=2,
        ),
    ]
