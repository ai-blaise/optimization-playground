...
The `ai-blaise/optimization-playground` fork includes additional opt-in paths for the REAP DeepSeek V3.2 lane, including the current `BlaiseAI/DeepSeek-V3.2-REAP-345B-NVFP4-W4A4KV4-IndexerK8-FP8-GatedNorm-G1` deployment target:
...
- **TokenSpeed MLA for NSA**: TokenSpeed composes with the REAP NSA stack as an NSA dense-MLA sub-backend. Keep `--attention-backend nsa` and use `--nsa-prefill-backend tokenspeed_mla` and/or `--nsa-decode-backend tokenspeed_mla` with HiSparse, IndexCache, dense TurboQuant, and LayerSplit. If `--attention-backend tokenspeed_mla` is supplied for this DSA/NSA model, SGLang routes it through the NSA backend so the selected-page semantics remain active. The selected-page TokenSpeed path currently requires SM100 and BF16 NSA KV storage; dense TurboQuant still owns the compressed KV footprint and only selected rows are decompressed for TokenSpeed.

The Dynamo production profile for `BlaiseAI/DeepSeek-V3.2-REAP-345B-NVFP4-W4A4KV4-IndexerK8-FP8-GatedNorm-G1` is a 4-GPU prefill plus 4-GPU decode deployment. It enables DSA sparse attention, IndexCache, dense TurboQuant 2.5-bit KV, decode-side HiSparse, Dynamo KV-aware routing, and decode-only SMC-SD. Do not enable SGLang HiCache in that profile unless the HiSparse no-radix contract is deliberately reworked and revalidated.
...
`--nsa-indexer-mode indexcache-hisa` is also present for continued experimentation. The NVFP4 IndexCache+HISA integration is strict Compression Ratio = 4:1 with `--hisa-compression-ratio 4.0`, block size 128, and `m=16/32/64/128` selected blocks for 8K/16K/32K/64K prefixes at `index_topk=2048`. Evaluate this path only against ordinary NVFP4 IndexCache; no other baseline is in scope. Current B200 forward facts versus ordinary NVFP4 IndexCache are: 8192 tokens 0.360976 ms versus HISA 0.072824 ms, 4.9569x; 16384 tokens 0.531413 ms versus 0.191044 ms, 2.7816x; 32768 tokens 1.006156 ms versus 0.259645 ms, 3.8751x; 65536 tokens 1.781986 ms versus 0.429308 ms, 4.1508x.
