...
from sglang.srt.layers.attention.dsa.indexer_policy import get_indexcache_skip_flags
        ...
        if self.use_dsa:
            ...
            self.indexer = Indexer(
                ...
                dsa_indexer_mode=(
                    "vanilla"
                    if is_nextn
                    else getattr(config, "dsa_indexer_mode", getattr(config, "nsa_indexer_mode", "vanilla"))
                ),
            ...
            self.skip_topk, self.next_skip_topk = get_indexcache_skip_flags(
                mode=getattr(config, "dsa_indexer_mode", getattr(config, "nsa_indexer_mode", "vanilla")),
                layer_id=layer_id,
