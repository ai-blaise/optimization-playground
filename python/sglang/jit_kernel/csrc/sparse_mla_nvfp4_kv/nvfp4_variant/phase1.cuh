...
KernelTemplate<FWD_MODE, D_QK>::sparse_attn_fwd_kernel_devfunc(const ArgT &params, const TmaParams &tma_params) {
    ...
    if (warp_idx == 0 && elect_one_sync()) {
        ...
        for (int i = 0; i < NUM_INDEX_BUFS; ++i) {
            smem.bar_valid_coord_full[i].init(IS_PREFILL ? B_TOPK/8 : 32);
            smem.bar_valid_coord_empty[i].init(IS_PREFILL ? 128 : (128 + (cta_idx==1) + 2 + 128));
        }
        if constexpr (IS_DECODE) {
            CUTE_UNROLL
            for (int i = 0; i < NUM_RAW_K_BUFS; ++i) {
                smem.bar_KV_full[i].init(1);
                smem.bar_KV_empty[i].init(128);
            }
        }
        fence_barrier_init();
    }
    ...
    if (warpgroup_idx == 0) {
        ...
        if constexpr (!IS_DECODE) {
            ...
            // ─────────────────────────────────────────────────────────────────
            // NVFP4 native-UMMA path: WG1 no longer dequants. The packed FP4
            // K values and their per-block E4M3 scales are already loaded by
            // the TMA loader warps into smem.K[k_buf_idx] + smem.K_scales[buf].
            // WG1's only remaining job in decode mode is the coord-prep step
            // that translates topk_indices -> per-token TMA coordinates +
            // validity masks (this is the work the FP8 path also did, but
            // wrapped inside the dequant lambda).
            //
            // The math warpgroup (WG2) consumes packed FP4 + E4M3 scales
            // directly via SM_100 `tcgen05.mma.kind::f8f6f4.block_scale_vec::1X`.
            // See nvfp4_umma_descriptors.cuh for the UMMA atoms.
            // ─────────────────────────────────────────────────────────────────
            auto launch_coord_prep_wg = [&]() {
                run_outer_loop([&](const OuterloopArgs &args) {
                    CUTE_NO_UNROLL
                    for (int block_idx = args.start_block_idx; block_idx < args.end_block_idx; ++block_idx) {
                        auto [index_buf_idx, index_bar_phase] = rs.get<NUM_INDEX_BUFS>();

                        // Wait until the previous coord buffer has been consumed.
                        smem.bar_valid_coord_empty[index_buf_idx].wait(index_bar_phase^1);

                        // The full coord-prep logic (translating topk_indices ->
                        // tma_coord[] + is_k_valid[]) lives in the helper warps
                        // (warp_idx >= 9, see further down in the kernel). WG1's
                        // job here is just to advance the ring buffer state and
                        // signal that the slot is ready for the TMA loader.
                        if (elect_one_sync()) {
                            smem.bar_valid_coord_full[index_buf_idx].arrive();
                        }
                        rs.update();
                    }
                });
            };
            launch_coord_prep_wg();
        }
        ...
        if (warp_idx == 8 && cta_idx == 0 && elect_one_sync()) {
            ...
            // NVFP4 native UMMA: build block-scaled instr_desc that ties the
            // per-block E4M3 scale tensor to the matmul. The descriptor is
            // re-built per iteration with the current k_buf_idx's scale-SMEM
            // address — issued just before the utcmma call below.
            // FOLLOWUP: thread `nvfp4_p_desc`/`nvfp4_o_desc` through utcmma_ts/
            // utcmma_ss in kerutils — the FP8 path uses kerutils helpers that
            // dispatch on TiledMMA's atom type, but the block-scale variant
            // wants a third argument (scale-tensor SMEM offset). Once
            // kerutils gains a `utcmma_ts_block_scaled` overload, pass the
            // descriptor here.
            ...
            if constexpr (IS_DECODE) {
                ...
                run_outer_loop([&](const OuterloopArgs &args) {
                    ...
                    for (int block_idx = args.start_block_idx; block_idx < args.end_block_idx; ++block_idx) {
                        auto [index_buf_idx, index_bar_phase] = rs.get<NUM_INDEX_BUFS>();
                        auto [k_buf_idx, k_bar_phase] = rs.get<NUM_K_BUFS>();
                        smem.bar_valid_coord_full[index_buf_idx].wait(index_bar_phase);
                        smem.bar_KV_empty[k_buf_idx].wait(k_bar_phase^1);
                        CUTE_UNROLL
                        ...
                        smem.bar_valid_coord_empty[index_buf_idx].arrive();
                        rs.update();
                    }
                });
            }
            ...
            if constexpr (IS_PREFILL) {
                if (lane_idx < B_TOPK/8) {
                    run_outer_loop([&](const OuterloopArgs &args) {
                        ...
                        for (int k = args.start_block_idx; k < args.end_block_idx; ++k) {
                            ...
                            smem.bar_valid_coord_empty[indices_buf_idx].wait(indices_bar_phase^1);
                            smem.is_k_valid[indices_buf_idx][lane_idx] = k_validness_mask;
                            smem.bar_valid_coord_full[indices_buf_idx].arrive();
                ...
                run_outer_loop([&](const OuterloopArgs &args) {
                    ...
                    auto process_one_block = [&](int block_idx, auto is_extra_block_t) {
                        ...
                        *(uint64_t*)(smem.K_scales[index_buf_idx] + lane_idx*2) = *(uint64_t*)scales;
            ...
            if constexpr (IS_PREFILL) {
                ...
                run_outer_loop([&](const OuterloopArgs &args) {
                    ...
                    for (int block_idx = args.start_block_idx; block_idx < args.end_block_idx; ++block_idx) {
                        ...
                        auto [index_buf_idx, index_bar_phase] = rs.get<NUM_INDEX_BUFS>();
                        smem.bar_valid_coord_full[index_buf_idx].wait(index_bar_phase);
                        smem.bar_KV_empty[raw_k_buf_idx].wait(raw_k_bar_phase^1);
                        ...
                        for (int row = (warp_idx == 10 ? 0 : 4); row < B_TOPK; row += 8) {
                            ...
                            ku::tma_gather4(
                                ...
                                smem.bar_KV_full[raw_k_buf_idx],
                                smem.K[raw_k_buf_idx].data() + row*(D_K/2),
                                cta_idx*(D_K/2),
                                cur_indices,
                                (int64_t)TMA::CacheHintSm90::EVICT_LAST
                            );
                        ...
                        if (warp_idx == 10) {
                            smem.bar_KV_full[raw_k_buf_idx].arrive_and_expect_tx(B_TOPK*(D_K/2)*sizeof(fp8_e4m3));
                        }
                        smem.bar_valid_coord_empty[index_buf_idx].arrive();
                        rs.update();
                    }
        ...
        run_outer_loop([&](const OuterloopArgs &args) {
            ...
            for (int k = args.start_block_idx; k < args.end_block_idx; ++k) {
                ...
                smem.bar_valid_coord_full[indices_buf_idx].wait(indices_bar_phase);
                ...
                smem.bar_valid_coord_empty[indices_buf_idx].arrive();
