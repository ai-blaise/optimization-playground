...
class ServerArgs:
            ...
            if is_deepseek_dsa(hf_config):  # DeepSeek 3.2/GLM 5
                ...
                if not is_npu() and not is_xpu():  # CUDA or ROCm GPU
                    if self.enable_dsa_prefill_context_parallel:
                        ...
                        # NOTE(ai-blaise merge): upstream forces
                        # attn_cp_size = tp_size // dp_size unconditionally and
                        # always disables piecewise CG under DSA CP. We retain
                        # the configurable attn_cp_size (with a divisibility
                        # check) but pick up the piecewise CG disable, which
                        # is required for upstream's DSACPLayerCommunicator
                        # path (no attn-TP all-reduce before dense FFN).
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        # NOTE(ai-blaise merge): upstream PR added --dsa-topk-backend alongside
        # our existing --dsa-* indexer args; compose both.
        parser.add_argument(
            "--dsa-topk-backend",
        ...
        parser.add_argument(
            ...
            help=(
                ...
                "NGRAM, DFLASH, SMC, FROZEN_KV_MTP. Or any name registered via "
        ...
        parser.add_argument(
            ...
            help=(
                "Capture a second set of CUDA graphs without LoRA hooks. "
