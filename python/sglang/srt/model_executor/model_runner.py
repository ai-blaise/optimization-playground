...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _dummy_run(self, batch_size: int, run_ctx=None):
        ...
        num_tokens_per_bs = 1
        if self.spec_algorithm.is_speculative() and (
            not self.spec_algorithm.is_smc() or not self.is_draft_worker
        ):
    ...
    def _forward_raw(
        ...
        with ctx_mgr:
            # Check whether we can run cuda graph for this forward. ai-blaise
            # adds a `forward_batch.disable_graph_runner` per-batch override
            # so individual call sites (e.g. SMC draft tail) can opt out
            # without touching the runner config.
            mode_check = (
