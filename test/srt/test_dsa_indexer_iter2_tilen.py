"""Correctness regression test for the iter2 DSA NVFP4 indexer tile-N
candidate_score kernel.

Verifies that ``hisa_candidate_score_tilen_indexer_cache_nvfp4`` (kTileN=8
candidates per CTA) produces logits and candidate_indices that match the
iter1 per-candidate ``hisa_candidate_score_indexer_cache_nvfp4`` kernel
within FP32 reduction-order round-off on the production shape grid.

This exercises the path used by ``nvfp4_hisa_indexer_paged_torch`` when
``SGLANG_NSA_NVFP4_HISA_CANDIDATE_SCORE_TILEN`` is True (the iter2 default
in the with-higgs deploy where DeepGEMM ``fp8_fp4_mqa_logits`` is absent).
"""
...
class TestDSAIndexerIter2CandidateScoreTileN(unittest.TestCase):
    """Tile-N kernel == iter1 per-candidate kernel modulo FP32 round-off."""
    ...
    def _build_case(self, batch, n_heads, head_dim, prefix, page=64, block=128, seed=2027):
        ...
        # iter1's exact production schedule with compression_ratio=4.0
    ...
    def _run(self, batch, n_heads, head_dim, prefix):
        ...
        iter1_logits, iter1_indices = hisa_candidate_score_indexer_cache_nvfp4(
        ...
        self.assertTrue(
            torch.equal(iter1_indices, tilen_indices),
            f"candidate_indices disagree on batch={batch} n_heads={n_heads} "
            f"prefix={prefix} eff_block_topk={eff}",
        )
        # -INFINITY pattern must match exactly.
        self.assertTrue(
            torch.equal(torch.isneginf(iter1_logits), torch.isneginf(tilen_logits)),
            f"-INFINITY pattern disagrees on batch={batch} n_heads={n_heads} "
            f"prefix={prefix}",
        )
        # Finite logits: allow a small FP32 reduction-order tolerance, same
        # bar as the iter1 fused-head block_score regression test.
        mask = torch.isfinite(iter1_logits) & torch.isfinite(tilen_logits)
        diff = (iter1_logits[mask] - tilen_logits[mask]).abs()
        base = iter1_logits[mask].abs().clamp_min(1e-6)
    ...
    def test_n_heads_64_fast_path(self):
        # candidate_len = 8 * 128 = 1024 == index_topk; tile-N covers
        # tile_blocks = ceil(1024 / 8) = 128 blocks per row.
        self._run(batch=8, n_heads=64, head_dim=128, prefix=4096)

    def test_n_heads_64_candidate_path_2048(self):
        # candidate_len = 16 * 128 = 2048; tile_blocks = 256.
    ...
    def test_n_heads_8_legacy_path(self):
        # n_heads = 8 exercises the heads_per_warp = 1 fused-head path in
        # the tile-N kernel (same code path; just a smaller per-warp head
        # count). This is the TP-shard config.
        self._run(batch=4, n_heads=8, head_dim=128, prefix=4096)

    def test_batched_production_grid(self):
        # Match the iter1 perf table's largest production shape.
