...
class DeepseekSparseAttnBackend(
    ...
    def forward_extend(
        ...
        if (
            dsa_impl in ("fa3", "flashmla_kv", "flashmla_sparse", "tilelang")
    ...
    def forward_decode(
        ...
        elif self.dsa_decode_impl == "tokenspeed_mla":
            if q_rope is not None:
                q_all = concat_mla_absorb_q_general(q_nope, q_rope)
            return self._forward_tokenspeed_mla_selected(
                q_all=q_all,
    ...
    def _forward_flashmla_kv(
        ...
        if not self.dsa_kv_cache_store_fp8 and not prequantized_fp8:
    ...
    def _pack_tokenspeed_selected_kv(
        ...
        if page_table_1.dim() != 2:
            raise ValueError("TokenSpeed DSA expects a 2D selected page table.")
