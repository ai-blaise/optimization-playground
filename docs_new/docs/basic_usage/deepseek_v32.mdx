...
- **GatedNorm Blackwell forward**: On Blackwell GPUs, the BF16 GatedNorm inference helper dispatches to `sgl_kernel.gated_norm_cute_forward` when the local `sgl-kernel` wheel is built with the registered op. It uses the existing torch-MM fallback for documented large-shape cases and keeps `SGLANG_GATED_NORM_USE_TRITON=1` / `SGLANG_GATED_NORM_DISABLE_CUTE=1` for debugging or compatibility.
