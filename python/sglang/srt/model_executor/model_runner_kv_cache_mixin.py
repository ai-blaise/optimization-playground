...
from sglang.srt.mem_cache.hisparse_memory_pool import (
    ...
    HiSparseHiggsDense2BitDSATokenToKVPool,
    HiSparseDSATokenToKVPool,
    HiSparseTurboQuantDSATokenToKVPool,
...
class ModelRunnerKVCacheMixin:
    ...
    def _init_pools(self: ModelRunner):
        ...
        if is_dsv4_model:
            ...
            dsa_pool_kwargs = {
                ...
                "end_layer": self.end_layer,
                "index_head_dim": get_dsa_index_head_dim(self.model_config.hf_config),
                "indexer_quantization": get_dsa_indexer_quant_method(
                    self.server_args
                ),
            }
            if self.enable_hisparse:
                ...
                dsa_pool_kwargs["host_to_device_ratio"] = (
            ...
            if self.server_args.enable_turboquant_dense_kv_cache:
                ...
                pool_cls = (
                    HiSparseTurboQuantDSATokenToKVPool
                    if self.enable_hisparse
                    else TurboQuantDSATokenToKVPool
                )
                ...
                pool_cls = (
                    HiSparseHiggsDense2BitDSATokenToKVPool
                    if self.enable_hisparse
                    else HiggsDense2BitDSATokenToKVPool
                )
                self.token_to_kv_pool = pool_cls(
                    **dsa_pool_kwargs,
                ...
                self.token_to_kv_pool = HiSparseDSATokenToKVPool(**dsa_pool_kwargs)
            else:
                self.token_to_kv_pool = DSATokenToKVPool(**dsa_pool_kwargs)
        elif self.use_mla_backend and not self.mambaish_config:
    ...
    def _apply_token_constraints(self: ModelRunner, token_capacity: int) -> int:
        ...
        if (
            self.server_args.dsa_prefill_cp_kv_storage_mode == "layersplit"
