...
class Scheduler(
    ...
    def init_invariant_checker(self) -> None:
        # NOTE(ai-blaise merge): upstream extracted the inline constructor
        # into this helper. Extend it with the ai-blaise SMC accessors so
        # the invariant checker can observe SMC manager / resampler state
        # alongside the upstream args. SchedulerInvariantChecker accepts
        # **kwargs for these (see scheduler_components/invariant_checker.py).
        self.invariant_checker = SchedulerInvariantChecker(
            ...
            get_last_batch=lambda: self.last_batch,
            get_running_batch=lambda: self.running_batch,
            get_smc_manager=lambda: self.smc_manager,
            get_smc_resampler=lambda: self.smc_resampler,
        )
    ...
    def run_batch(
        ...
        if self.is_generation:
            if self.enable_overlap:
                # SMC: explicit refresh; spec_v2: future_map self-gates.
                # `future_map.resolve_seq_lens_cpu` is the upstream entry
                # point that no-ops for non-spec_v2, so it's safe to call
                # here for all non-SMC paths.
                if self.spec_algorithm.is_smc():
                ...
                with self._overlap_forward_isolation(batch):
                    ...
                    # SMC has its own direct verified-id handoff.
