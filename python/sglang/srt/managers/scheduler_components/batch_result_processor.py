...
class SchedulerBatchResultProcessor:
    ...
    def process_batch_result_prefill(
        ...
        if smc_prefill_stream_candidates:
            ...
            self.output_streamer.stream_output(smc_prefill_stream_candidates, False)
        ...
        for i, req in smc_parent_group_jobs:
            assert self.scheduler.smc_resampler is not None
            if hasattr(self.scheduler.smc_resampler, "enqueue_parent_group"):
                self.scheduler.smc_resampler.enqueue_parent_group(req)
            else:
                self.model_worker.materialize_smc_parent_draft_prefix(req)
                self.scheduler.smc_manager.create_group(req, self.scheduler)
                _release_smc_parent_req(
                    req,
                ...
                self.scheduler.smc_resampler.enqueue_group_for_running(req.rid)
            if smc_probe_enabled:
                append_smc_probe_record(
                    {
    ...
    def _resolve_spec_overlap_tokens(
        ...
        synchronous SMC path does not, so this helper also tolerates GPU
        tensors there.
        """
        ...
        assert stride is not None, "spec result missing speculative_num_draft_tokens"
    ...
    def process_batch_result_decode(
        ...
        for i, req in enumerate(batch.reqs):
            ...
            if (self.enable_overlap or self.enable_overlap_mlx) and (
                ...
                if batch.spec_algorithm.is_smc():
                    ...
                    if req.finished() and self.scheduler.smc_manager is not None:
                        self.scheduler.smc_manager.on_particle_finished(req)
                    if req.is_retracted:
                        _release_internal_req(
                            req,
                            req_to_token_pool=self.req_to_token_pool,
            ...
            if batch.spec_algorithm.is_smc() and self.scheduler.smc_manager is not None:
        ...
        if batch.spec_algorithm.is_smc() and self.scheduler.smc_resampler is not None:
            ...
            if smc_step_reqs:
                ...
                smc_finalized_reqs.extend(
                    self.scheduler.smc_resampler.on_batch_done(
                        smc_step_reqs,
                        step_logprob_diffs,
                        group_spans=(
        ...
        if batch.spec_algorithm.is_smc() and not self.enable_overlap and batch.reqs:
            ...
            batch.seq_lens = refreshed_seq_lens_cpu.to(device=self.scheduler.device)
        ...
        if smc_finalized_reqs:
            for req in smc_finalized_reqs:
                req.time_stats.set_completion_time()
            self.output_streamer.stream_output(smc_finalized_reqs, False)
        self.token_to_kv_pool_allocator.free_group_end()
    ...
    def _apply_decode_logprobs(
        ...
        *,
        ...
        if batch.is_spec_v2 or batch.spec_algorithm.is_smc():
        ...
        *,
        ...
        try:
            if batch.spec_algorithm.is_none():
                ...
                req.grammar.accept_token(next_token_id)
            elif batch.is_spec_v2 or batch.spec_algorithm.is_smc():
