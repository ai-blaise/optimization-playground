...
def _require_deep_gemm_kernel(name: str):
...
def _deep_gemm_fp4_mqa_logits(
    ...
    *,
    ...
    kv_values, kv_scales = kv_fp4
    kernel = _require_deep_gemm_kernel("fp8_fp4_mqa_logits")
    logits = kernel(
        (q_values.view(torch.int8), q_scales),
        (kv_values.view(torch.int8), kv_scales),
        weights,
        ks,
        ke,
        clean_logits=clean_logits,
...
class Indexer(MultiPlatformOp):
    ...
    def __init__(
        ...
        self.dsa_indexer_mode = dsa_indexer_mode
        ...
        self.enable_dsa_nvfp4_hisa = bool(
            getattr(
                server_args,
                "enable_dsa_nvfp4_hisa",
    ...
    def _quantize_query_for_indexer(
        self,
        query: torch.Tensor,
        forward_batch: ForwardBatch,
        act_quant,
    ):
        if not self._uses_nvfp4_indexer(forward_batch):
    ...
    @staticmethod
    def _split_indexer_query(q, split_size: int):
        if isinstance(q, tuple):
            values_prev, values_next = torch.split(q[0], split_size, dim=0)
            scales_prev, scales_next = torch.split(q[1], split_size, dim=0)
            return (values_prev, scales_prev), (values_next, scales_next)
        return torch.split(q, split_size, dim=0)
    ...
    def _get_topk_paged(
        ...
        if is_nvfp4:
            q_values, q_scales = q_fp8
            assert len(q_values.shape) == 3
            q_fp8 = (
                q_values.view(torch.int8).unsqueeze(1),
                q_scales.unsqueeze(1),
            )
        else:
        ...
        if _is_hip:
            ...
            if is_nvfp4:
                kernel = _require_deep_gemm_kernel("fp8_fp4_paged_mqa_logits")
                logits = kernel(
                    (q_fp8[0][:q_offset], q_fp8[1][:q_offset]),
                    kv_cache_fp8,
                    weights[:q_offset],
                    seqlens_32_2d,
                    block_tables,
        ...
        q_len = q_fp8[0].shape[0] if is_nvfp4 else q_fp8.shape[0]
        if not _is_hip and q_offset < q_len:
            pad_len = q_len - q_offset
            padding = torch.full(
    ...
    def _should_use_hisa_paged(
        ...
        if not uses_hisa(self.dsa_indexer_mode):
            return False
        if self.hisa_execution_mode != "optimized":
            return False
        if not _is_cuda or _is_fp8_fnuz or not envs.SGLANG_DSA_FUSE_TOPK.get():
            return False
        ...
        max_seq_len = _get_hisa_decision_max_seq_len(
            forward_batch, metadata, seqlens_32
        )
    ...
    def _should_use_hisa_nvfp4_paged(
        ...
        if not self.enable_dsa_nvfp4_hisa:
            return False
        if not uses_hisa(self.dsa_indexer_mode):
            return False
        if self.hisa_execution_mode != "optimized":
    ...
    def _should_use_hisa_nvfp4_extend(
        self,
        forward_batch: ForwardBatch,
        metadata: BaseIndexerMetadata,
        seqlens_32: torch.Tensor,
        ...
        if max_seq_len is None:
            return False
        if self.hisa_compression_ratio > 0:
            if not _has_deep_gemm_kernel("fp8_fp4_mqa_logits"):
                return max_seq_len > 0
            return max_seq_len > self.index_topk
        min_seq_len = max(
            self.hisa_min_seq_len,
    ...
    def _get_topk_hisa_paged(
        ...
        from sglang.srt.layers.attention.dsa.hisa_tilelang_kernels import (
            hisa_indexer_paged,
        )
    ...
    def _get_topk_hisa_nvfp4_paged(
        ...
        if topk_offset is not None:
            topk_offset = topk_offset[:q_offset].to(
                device=q_values.device, dtype=torch.int32
            )
            if topk_offset.ndim == 1:
                topk_offset = topk_offset.unsqueeze(1)
            topk_result[:q_offset] = torch.where(
                topk_relative < 0,
                torch.full_like(topk_relative, -1),
                topk_relative.to(torch.int32) + topk_offset,
            )
            return topk_result
    ...
    def _should_use_hisa_extend(
        ...
        if not _is_cuda or _is_fp8_fnuz or not fuse_topk:
            return False
        if self.dsa_enable_prefill_cp and not self.enable_hisparse:
            return False
    ...
    def _get_topk_ragged(
        ...
        q_offset = ks.shape[0]
        hisa_extend_ok = (not is_nvfp4) and self._should_use_hisa_extend(
            forward_batch, metadata
        )
        hisa_nvfp4_extend_ok = is_nvfp4 and self._should_use_hisa_nvfp4_extend(
            forward_batch, metadata, metadata.get_indexer_seq_len()
        )
        ...
        if hisa_nvfp4_extend_ok:
            ...
            if result is not None:
                _hisa_profile_end(
                    ...
                    batch_size=int(batch_size),
                    page_size=int(page_size),
                    has_dense_nvfp4_kernel=bool(
                        _has_deep_gemm_kernel("fp8_fp4_mqa_logits")
                    ),
                )
                return result
            _hisa_profile_end(
                "hisa_nvfp4_extend_skip",
                profile_start,
                device,
                layer_id=int(layer_id),
                forward_mode=str(forward_batch.forward_mode),
        ...
        if is_nvfp4:
            ...
            k_scale = k_scale.view(torch.int32).reshape(-1)
        ...
        if not need_chunk:
            ...
            _hisa_profile_end(
                ...
                hisa_eligible=bool(hisa_extend_ok or hisa_nvfp4_extend_ok),
                hisa_boundary_q=int(hisa_boundary_q),
                chunked=False,
            )
            return topk_result
        ...
        while start < q_offset:
            ...
            with self._with_real_sm_count():
                if is_nvfp4:
                    logits_chunk = _deep_gemm_fp4_mqa_logits(
                        (q_fp8[0][start:end], q_fp8[1][start:end]),
                        kv_fp8,
                        weights[start:end],
                        ks[start:end],
                        ke[start:end],
    ...
    def _get_topk_ragged_with_cp(
        ...
        if cp_index is not None:
            ...
            if is_nvfp4:
                k_fp8 = torch.cat(k_fp8_list, dim=0).view(torch.int8)
                k_scale = torch.cat(k_scale_list, dim=0).view(torch.int32).reshape(-1)
            else:
    ...
    def _store_index_k_cache(
        ...
        *,
        ...
        # checkpoint declares one (see should_use_dsa_fused_store);
        # otherwise falls back to the historical platform/dtype auto-detect.
        if should_use_dsa_fused_store(
            ...
            if (
                ...
                if (
                    ...
                    q_split = (
                        ...
                        else (q_fp8.shape[0] + 1) // 2
                    )
                    q_fp8_prev, q_fp8_next = self._split_indexer_query(
                        q_fp8, q_split
                    )
