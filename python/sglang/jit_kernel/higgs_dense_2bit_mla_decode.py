...
* :func:`higgs_dense_2bit_mla_decode` — single-pass decode (grid =
  ``(num_rows, num_heads)``). Optimal when ``num_rows * num_heads``
...
* :func:`higgs_dense_2bit_mla_decode_split` — split-K decode (grid =
  ``(num_rows, num_heads, num_splits)``). Each split processes
  ``ceil(topk / num_splits)`` slots; a merge kernel combines the
  partial ``(m, l, acc)`` tuples and runs the inverse FWHT_512.
  Restores throughput at small batch sizes when the topk loop would
  otherwise starve SMs.
"""
...
def higgs_dense_2bit_mla_decode(
    ...
    """Single-pass fused MLA decode against a 2-bit HIGGS packed KV cache.
