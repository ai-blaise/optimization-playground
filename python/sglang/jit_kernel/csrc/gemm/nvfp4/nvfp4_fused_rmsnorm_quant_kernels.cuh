...
void invokeFusedRMSNormFP4QuantLinear(int m, int n, T* input, T* residual,
                                      ...
                                      DLDevice device) {
  // 8 elts/thread, block size capped at 512 to match the standalone
  // linear quant kernel.
  ...
  // Static guarantee on per-thread vec count.
  ...
  // Dispatch the 2x2 specialization grid (UE8M0 × cast_x_before_out_mul).
