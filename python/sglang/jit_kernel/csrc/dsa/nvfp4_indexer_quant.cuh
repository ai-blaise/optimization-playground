...
//
// The base hisa_block_score kernel uses grid (max_blocks, q_rows) so
// every (row, block_id) pair gets its own CTA. At the production
// 64/32768 cell that is 256 * 64 = 16384 CTAs, and every CTA
// re-dequantizes the SAME Q[row, *] NVFP4 tile (32 NVFP4 dequants per
// CTA across 8 warps * 8 heads per warp * 1 lane-quartet per head =
// 32 dequants per warp = 256 dequants per CTA). Total redundant
// dequants per launch: 16384 * 256 = 4.19M; only 64 * 128 = 8192 are
// actually unique. ~512x oversampling.
//
// This variant flips the grid to (q_rows,) and predecodes the
// Q[64 heads][128 dim] tile to fp32 into smem_q_fp32[64][128] = 32 KB
// SMEM once per CTA. The block-loop then iterates over all max_blocks
// for the row, doing only the per-block reps load + dot + weighted
// reduction.
//
// SMEM budget (sm_100a default 48 KB):
...
//   ------------------------------------------------
//   total                              ~ 33792 B  (~33 KB)
// Comfortably under the default cap; no cudaFuncSetAttribute.
//
// Trade-off: per-CTA wall time grows ~max_blocks-fold (256x at 64/32768)
// but CTA count drops ~max_blocks-fold. Net: launch + per-CTA fixed
// overhead amortizes across all blocks of one row, and the Q dequant
// is done exactly once per row (vs max_blocks times).
//
// Expected gain (at production 64/32768): ~3x block_score speedup
// (162us isolated -> ~50-60us projected). At index_topk_freq=4
// amortization this saves ~25-30us per decode step.
//
// Correctness: identical to the base block_score kernel (same dot
// computation, same warp_sum reduction order, same weighted reduction
// epilogue, same -INFINITY padding for block_id >= block_count).
__global__ void hisa_block_score_persistent_indexer_cache_nvfp4(
    const __grid_constant__ NVFP4HISABlockScoreParam param) {
  constexpr uint32_t kHISABlockSize = 128;
  constexpr uint32_t kMaxWarps = 8;
  constexpr uint32_t kMaxHeads = 64;

  // Q pre-decoded into fp32, indexed [head][dim].
  ...
  // Stage weights[n_heads] to SMEM once. Tail heads (h >= n_heads) zero
  // so the unrolled reduction below uses uniform indexing.
  if (tid < kMaxHeads) {
  ...
  // Per-block loop. Each iter computes block_scores[row, block_id]
  // using the predecoded Q and per-block reps.
  ...
  for (uint32_t block_id = 0; block_id < block_count; ++block_id) {
    ...
    // Stage 3: per-warp head dispatch. Uses pre-decoded Q from SMEM
    // (no NVFP4 dequant in the inner loop).
    #pragma unroll
    ...
    // Stage 4: single-warp weighted reduction. Same order as the base
    // kernel: sum over h of fmaxf(head_dot[h], 0) * weights[h].
    if (warp_id == 0) {
      float partial = 0.0f;
      for (uint32_t h = lane; h < param.n_heads; h += 32) {
    ...
    // No __syncthreads at end-of-iter: next iter's Stage 1 syncs after
    // smem_reps load, which closes the cur-iter Stage 4 -> next-iter
    // Stage 1 race because smem_reps is the only SMEM buffer mutated
    // between syncs (head_dot is written by all warps in Stage 3 and
    // read by warp 0 in Stage 4; smem_q_fp32 and smem_weights are
    // read-only after the predecode sync; smem_reps is written by
    // tid < 128 in Stage 1 and read by all lanes in Stage 2).
  }

  // Pad block_id >= block_count with -INFINITY. The base kernel
  // implicitly does this via the early-return in each CTA; here we
  // need an explicit fill loop since the persistent CTA owns all
  // max_blocks slots for this row.
  for (uint32_t block_id = block_count + tid; block_id < param.max_blocks;
       block_id += blockDim.x) {
...
struct NVFP4IndexerQuantKernel {
  ...
  // iter7 PRIMARY: persistent block_score with Q-value pre-staging.
  static constexpr auto block_score_persistent_kernel =
  ...
  // iter7 PRIMARY: persistent block_score with Q-value pre-staging.
  // Same FFI as hisa_block_score; launches with grid (q_rows,) so each
  // CTA owns all max_blocks of one row and predecodes the Q tile once
  // into SMEM (saving ~512x redundant NVFP4 dequants at the production
  // 64/32768 cell).
  static void hisa_block_score_persistent(
      tvm::ffi::TensorView q_values,
      tvm::ffi::TensorView q_scales,
      tvm::ffi::TensorView reps,
      tvm::ffi::TensorView weights,
    ...
    LaunchKernel(dim3(params.q_rows), 256, device_.unwrap())(
        block_score_persistent_kernel, params);
  }
